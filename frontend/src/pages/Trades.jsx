...
export default function Trades() {
  ...
  const [filters, setFilters] = useState({ status: "", direction: "", symbol: "" });
      ...
      <div className="card p-3 flex flex-wrap items-center gap-2">
        ...
        {(filters.status || filters.direction || search) && (
          <button
            className="btn-ghost"
            onClick={() => {
              setFilters({ status: "", direction: "", symbol: "" });
              setSearch("");
            }}
          >
            <X size={14} /> Clear
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            ...
            <tbody>
              {loading ? (
                <tr>
                  <td colSpan={11} className="text-center py-10 text-slate-500">Loading…</td>
                </tr>
              ) : visible.length === 0 ? (
                <tr>
                  <td colSpan={11} className="text-center py-10 text-slate-500">No trades match.</td>
                </tr>
              ) : (
                visible.map((t) => (
                  <tr key={t.id} className="border-b border-slate-800/50 last:border-0 hover:bg-slate-800/30">
                    ...
                    <td className="py-3 px-4 text-right text-slate-400">{fmtNumber(t.quantity, 0)}</td>
                    <td className="py-3 px-4 text-right text-slate-400">{fmtNumber(t.entry_price)}</td>
                    <td className="py-3 px-4 text-right text-slate-400">{t.exit_price ? fmtNumber(t.exit_price) : "—"}</td>
                    <td className={`py-3 px-4 text-right font-medium ${pnlColor(t.net_pnl)}`}>
                      {t.net_pnl === null ? "—" : fmtMoney(t.net_pnl)}
                      {t.return_pct !== null && (
                        <span className="block text-[11px] text-slate-500">{fmtPct(t.return_pct)}</span>
                      )}
                    </td>
                    <td className={`py-3 px-4 text-right ${pnlColor(t.r_multiple)}`}>
                      {t.r_multiple === null ? "—" : `${fmtNumber(t.r_multiple)}R`}
                    </td>
                    <td className="py-3 px-4 text-slate-400">{t.setup || "—"}</td>
                    <td className="py-3 px-4 text-slate-500 whitespace-nowrap">{fmtDate(t.entry_date)}</td>
                    <td className="py-3 px-4">
                      <div className="flex items-center justify-end gap-1">
                        <button onClick={() => openEdit(t)} className="p-1.5 rounded-md hover:bg-slate-700 text-slate-400 hover:text-slate-200">
                          <Pencil size={15} />
                        </button>
                        <button onClick={() => remove(t)} className="p-1.5 rounded-md hover:bg-rose-600/20 text-slate-400 hover:text-rose-400">
                          <Trash2 size={15} />
                        </button>
                      </div>
                    </td>
